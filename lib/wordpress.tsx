...
export async function getPosts(params?: {
  ...
  try {
    ...
    console.log("[v0] Fetching WordPress posts from:", `${WORDPRESS_API_URL}?${searchParams}`)
    ...
    if (!response.ok) {
      console.error("[v0] WordPress API response not ok:", response.status, response.statusText)
...
export async function getCategories(): Promise<WordPressCategory[]> {
  try {
    if (!WORDPRESS_API_URL) {
      console.warn("[v0] WordPress API URL not configured. Using mock categories.")
      return getMockCategories()
    }

    console.log("[v0] Fetching WordPress categories from:", `${WORDPRESS_API_URL}`)
    ...
    if (!response.ok) {
      console.error("[v0] WordPress categories API response not ok:", response.status, response.statusText)
      throw new Error(`Failed to fetch categories: ${response.status} ${response.statusText}`)
    }

    const categories = await response.json()
    console.log("[v0] Successfully fetched", categories.length, "WordPress categories")
    return categories
  } catch (error) {
...
export async function getPostBySlug(slug: string): Promise<WordPressPost | null> {
  try {
    ...
    const response = await fetch(`${WORDPRESS_API_URL}?slug=${slug}&_embed=true`, {
      next: { revalidate: 300 },
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
...
function getMockPosts(params?: {
  ...
  const mockPosts: WordPressPost[] = [
    {
      id: 1,
      title: { rendered: "The Ultimate Guide to White-Hat Link Building in 2024" },
      excerpt: {
        rendered:
      ...
      content: { rendered: "<p>Link building remains one of the most crucial aspects of SEO...</p>" },
    ...
    {
      id: 3,
      title: { rendered: "Content Marketing Strategies That Attract Natural Backlinks" },
      excerpt: {
        rendered:
          "Create content that naturally attracts high-quality backlinks from authoritative websites in your industry.",
      },
      content: { rendered: "<p>The best backlinks are earned, not built...</p>" },
  ...
  const filteredPosts = params?.search
    ? mockPosts.filter(
        (post) =>
          post.title.rendered.toLowerCase().includes(params.search!.toLowerCase()) ||
          post.excerpt.rendered.toLowerCase().includes(params.search!.toLowerCase()),
      )
    : mockPosts
